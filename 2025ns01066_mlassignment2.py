...
Streamlit App (converted from Google Colab)
"""
...
# Page config
# ─────────────────────────────────────────────
st.set_page_config(page_title="ML Assignment 2 — Student Performance",
                   layout="wide")

st.title("🎓 Student Performance Prediction — ML Assignment 2")
st.markdown("Upload the `StudentPerformance_Dataset.csv` file to begin.")

# ─────────────────────────────────────────────
# File Upload  (replaces google.colab files.upload)
# ─────────────────────────────────────────────
uploaded_file = st.file_uploader("Upload StudentPerformance_Dataset.csv",
                                  type=["csv"])
...
for label, count in counts.items():
    st.write(f'  {"Pass" if label==1 else "Fail"} ({label}) : {count} students ({count/len(df)*100:.1f}%)')
...
st.write("**Encoding applied:**", dict(zip(le.classes_, le.transform(le.classes_))))
...
# EDA — Distribution
# ─────────────────────────────────────────────
st.subheader("EDA — Target Variable Distribution")
...
st.write("**Correlation with Pass (target):**")
st.dataframe(df_processed.corr()['Pass'].drop('Pass').sort_values(ascending=False).round(3))
...
# ─────────────────────────────────────────────
# EDA — Pass Rate Analysis
# ─────────────────────────────────────────────
st.subheader("Pass Rate Analysis")
...
pass_rate_bar(axes[6], df['Extra_Coaching'].map({0:'No',1:'Yes'}), df['Pass'], 'Extra Coaching vs Pass Rate')
...
st.info("Training 8 models — this may take a moment...")
...
mdf = metrics_df  # keep alias for pickle

st.write("**Summary of Model Evaluation Metrics:**")
st.dataframe(metrics_df.style.format('{:.2%}'))  # replaces display()
...
for i, metric in enumerate(metrics_to_plot):
    sns.barplot(x=metrics_df.index, y=metrics_df[metric], ax=axes[i],
                palette='viridis', hue=metrics_df.index, legend=False)
...
# ─────────────────────────────────────────────
# Save Model Artifacts (download button replaces files.download)
# ─────────────────────────────────────────────
st.subheader("Save Model Artifacts")
...
# Live Prediction
# ─────────────────────────────────────────────
st.subheader("🔮 Live Student Prediction")
st.markdown("Adjust the sliders to predict whether a student will pass or fail.")
